...
pub fn get_order() -> Vec<GameStateSmall> {
    ...
    let mut counts: HashMap<PermaKey, usize> = HashMap::new();
    for game in &states {
        let key = PermaKey::new(GameState::from(*game));
...
fn create_order_rec(
    ...
    states: &mut BTreeSet<GameStateSmall>,
) {
